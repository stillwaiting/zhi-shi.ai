...
import { isMarkdownBodyChunkConnection, isMarkdownBodyChunkList, isMarkdownBodyChunkQuestionAnswers, isMarkdownBodyChunkTable, isMarkdownBodyChunkTextParagraph, MarkdownBody, MarkdownNode } from '../md/types';
...
export default ( { body }: BodyComponent ) => {
    return <div className='BodyComponent'>
        {body.content.map((chunk, contentIdx) => {
            ...
            } else if (isMarkdownBodyChunkConnection(chunk)) {
                return <BodyConnectedNodeComponent connectedNodeTitle={chunk.connectedNodeTitle} 
                    isAutogenerated={chunk.isAutogenerated} />;
            }
        })}
    </div>;
}
